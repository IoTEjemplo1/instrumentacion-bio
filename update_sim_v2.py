...
try:
    ...
    old_toolbar = r"""  <span style="font-weight:800;color:var(--p);display:flex;align-items:center;margin-right:8px;font-size: 0.95rem;">Casos preconfigurados.*?</span>
  ...
  <button id="pre60Hz">Interferencia Eléctrica</button>"""
    ...
    old_noise = r"""    <h3 style="display:flex;align-items:center;">6\. Ruido e interferencia.*?<label><input id="whiteOn".*?</label>
    ...
    <label><input id="mainsOn".*?</label>
    <select id="mainsF">.*?</select>
    ...
    content = re.sub(old_noise, new_noise, content, flags=re.DOTALL)
    ...
    old_signals = r"""function signals\(\)\{
...
\}"""
 ...
 return{fs,n,t,clean,input};
    ...
    content = re.sub(old_signals, new_signals, content, flags=re.DOTALL)

    # 8. Modify Hideal function
    old_hideal = r"""function Hideal\(f\)\{
...
\}"""
 ...
 if(state.filter==='low')return{m:A/Math.sqrt(1+w**(2*o)),p:-o*Math.atan(w)};
    ...
    content = re.sub(old_hideal, new_hideal, content, flags=re.DOTALL)

    # 9. Modify Circuit function SVG
    old_svg = r""" const low=state\.filter==='low';
 ...
 ${passiveStage}`
    ...
    # Also strip out the tooltips style inside circuit if they are there, the old_svg regex handles it because of .*?</style> but wait... 
    # my new_svg doesn't have the tooltip styles. Actually they were added dynamically, let's keep them removed from SVG to reduce clutter.

    # 10. Update buttons logic
    old_buttons = r"""\$\('preECG'\)\.onclick=\(\)=>\{.*?\};\n\$\('preLow'\)\.onclick=\(\)=>\{.*?\};\n\$\('preHigh'\)\.onclick=\(\)=>\{.*?\};\n\$\('preSaturation'\)\.onclick=\(\)=>\{.*?\};"""
