...
// Find initOscilloscopeActivity
const fnStart = code.indexOf('function initOscilloscopeActivity');
if (fnStart !== -1) {
  // Find the components list end
  const listEndSearch = '    }\n  ];\n\n  const triviaQuestions = [';
  const targetIdx = code.indexOf(listEndSearch, fnStart);
  
  if (targetIdx !== -1) {
    const replacement = newComponents + '\n\n  const triviaQuestions = [';
    code = code.substring(0, targetIdx) + replacement + code.substring(targetIdx + listEndSearch.length);
    fs.writeFileSync('js/components.js', code);
    console.log('Appended safely.');
  } else {
    console.log('Could not find list end inside function.');
  }
} else {
