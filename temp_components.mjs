    ...
    if (tabId === 'theory') {
      ...
      if (currentTopic.simulator) {
        ...
        embedsHTML = simulators.map(simFile => {
          ...
          let simDesc = '';
          if (simFile === 'simulador_circuitos_led.html') {
  ...
  const startPhase1 = () => {
    ...
    componentsList.forEach((comp, index) => {
      ...
      card.innerHTML = `
        ...
        <div class="fw-bold text-dark flex-grow-1" style="font-size: 0.75rem; color: #1e293b !important; line-height: 1;">
          ${comp.name}
        </div>
...
export function initDmmActivity(onAwardXp) {
  ...
  const componentsList = [
    {
      ...
      color: "#3b82f6",
      coords: { top: 18, left: 18, width: 60, height: 22 },
      number: "1"
    },
    {
      id: "encendido-dmm",
      ...
      color: "#ef4444",
      coords: { top: 83, left: 7, width: 6, height: 8 },
      number: "2"
    },
    {
      id: "teclado-medicion",
      ...
      color: "#10b981",
      coords: { top: 46, left: 20, width: 28, height: 18 },
      number: "3"
    },
    {
      ...
      color: "#f59e0b",
      coords: { top: 72, left: 20, width: 28, height: 18 },
      number: "4"
    },
    {
      ...
      color: "#8b5cf6",
      coords: { top: 38, left: 81, width: 10, height: 18 },
      number: "5"
    },
    {
      ...
      color: "#ec4899",
      coords: { top: 62, left: 81, width: 10, height: 18 },
      number: "6"
    }
  ];
  ...
  const startPhase1 = () => {
    ...
    shuffled.forEach(comp => {
      const card = document.createElement('div');
      card.className = 'osc-draggable-card shadow-sm';
      card.draggable = true;
      card.dataset.id = comp.id;
      
      const icon = document.createElement('div');
      icon.className = 'osc-card-icon';
      icon.innerHTML = '<span class="text-secondary fw-bold">?</span>';
      
      const text = document.createElement('span');
      text.className = 'fw-semibold small text-dark';
    ...
    componentsList.forEach(comp => {
      ...
      const badgeEl = document.createElement('div');
      badgeEl.className = 'osc-hotspot-badge text-dark';
      badgeEl.textContent = '?';
      hs.appendChild(badgeEl);
  ...
  const handleCorrectMatch = (comp, hsElement) => {
    const card = document.querySelector(`.osc-draggable-card[data-id="${comp.id}"]`);
    if (card) {
      card.classList.add('matched-card');
      card.draggable = false;
      const icon = card.querySelector('.osc-card-icon');
    ...
    const badgeEl = hsElement.querySelector('.osc-hotspot-badge');
    badgeEl.textContent = comp.number;
    badgeEl.style.backgroundColor = comp.color;
  ...
  const startPhase2 = () => {
    ...
    componentsList.forEach(comp => {
      ...
      const badgeEl = document.createElement('div');
      badgeEl.className = 'osc-hotspot-badge text-white';
      badgeEl.style.backgroundColor = comp.color;
      badgeEl.textContent = comp.number;
      hs.appendChild(badgeEl);
